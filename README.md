...
`xenith-build all` compiles the BIOS/UEFI bootloader, kernel, Rust userspace (including the graphical Files app, opt-in window smoke, native thread smoke, and bounded Win64 console host), and `user/c/xenith-c-demo.c` through `xenith-cc`/`xenith-asm`/`xenith-ld`. It packages those programs plus the source-built PE fixture and the seeded Windows `C:\` namespace, then writes the raw BIOS layout `build/xenith.img` and a hybrid ISO9660/El Torito image with BIOS hard-disk emulation plus a FAT16 EFI System Partition at `build/xenith.iso`. See [STATUS](docs/STATUS.md) for the separate runtime-proof boundary of each artifact.
...
Kernel and userspace require their separate custom targets and build-std flags; host tools use the native target. CI checks the complete workspace, runs host/kernel/bootloader tests and strict Clippy, builds every artifact with `xenith-build all`, then gates the idle desktop lifecycle, on-demand Files launch/navigation/create/delete/cleanup, the opt-in native shared-buffer client path, and native thread create/join/teardown as well as direct, packaged-image, BIOS-stage, UEFI, SMP, shell/coreutils, pipeline, C-toolchain, and debugger execution in Xenith's own emulator.

The Windows boundary is intentionally narrow. Xenith now mounts a writable, ASCII case-insensitive and case-preserving `/win` namespace, maps `C:\` to `/win/c`, seeds modern system/common/profile folders, and lets `xenith-winhost` open its executable through a bounded Windows drive path. The known-folder and environment builders are policy-only, and the namespace is volatile until persistent system-volume mounting lands. `xenith-winhost` still runs only a checked Win64 console image whose imports fit the documented bootstrap shim. It gives accepted code a dedicated bounded stack with an unmapped lower guard page, but it is a trusted bootstrap runner, not a sandbox: the PE executes inside the host process with its Xenith syscall authority and inherited descriptors. `xenith-winhost-core` catalogs 13 symbol-addressed, pointer-free NT policy services, but only `NtClose` is wired to a guest import; native Xenith threads do not implement Windows thread semantics. Registry, general guest file/process/synchronization APIs, `user32`/`gdi32`, COM, DirectX, .NET, installers, and WoW64 remain unsupported. `xenith-windrv-core` is policy-only and does not load or execute Windows drivers.

See [BUILD](docs/BUILD.md), [WINDOWS_FILESYSTEM](docs/WINDOWS_FILESYSTEM.md), [TOOLCHAIN](docs/TOOLCHAIN.md), [EMULATOR](docs/EMULATOR.md), [BOOT_PROTOCOL](docs/BOOT_PROTOCOL.md), [DESKTOP_FOUNDATION](docs/DESKTOP_FOUNDATION.md), and [STATUS](docs/STATUS.md). `STATUS.md` distinguishes compile-tested code from emulator-, firmware-, or hardware-tested behavior.
